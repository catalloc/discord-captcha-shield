...
## This project — Discord CAPTCHA Shield (multi-tenant)

A CAPTCHA + Discord OAuth verification gate. **One deployment shields many
Discord servers**, each with its own config and branding. Read `README.md` for
the user-facing model; this section is the architecture map for editing the code.

### Shape

- **One shared Discord app + bot** for all servers, in shared env
  (`DISCORD_CLIENT_ID` / `_SECRET` / `DISCORD_BOT_TOKEN`). Per-server values
  (role, branding, channels, alert tuning, optional Turnstile keys) live in the
  `captcha_guilds` SQLite table, **not** env.
- **Routing** (`mod.ts` `handleVerifyRequest`): per-server pages under
  `/g/{guildId}` (`` = portal, `/start`, `/og.png`); a single shared
  `/callback`; and the guarded `/admin/*` API. The OAuth `state` nonce carries
  the guild id, so **one** registered `/callback` redirect URI serves every
  server — never add per-guild redirect URIs to the OAuth routing.
- **Config flow**: `config.ts` splits `loadSharedEnv()` (secrets) from
  `loadGuildConfig(guildId)` (merges a `captcha_guilds` row with shared env →
  `VerifyConfig`). Tools use `loadGuildToolConfig()` (no Turnstile requirement).
  `guilds.ts` is the CRUD registry; `admin.ts` is the bearer-guarded HTTP layer
  over it. Branding resolves via `theme.ts` `resolveTheme()` (defaults) layered
  with a guild's overrides (`resolveGuildTheme()`).
- **Per-guild scoping**: state nonces + audit log carry `guild_id`; raid alerts
  namespace the guild into the alert `kind` (`raid:<guildId>`); blob keys in
  `store.ts` and the OG card in `og.ts` are keyed by guild id.

### Invariants — don't break these

- **Schema migrations: rename, never ALTER.** Per the SQLite guidance below,
  adding a column means a **new table name** (e.g. `captcha_verify_log_v2`)
  created fresh with the column, plus a one-time data copy where it matters — see
  `schema.ts`. Do not reintroduce `ALTER TABLE`. All table DDL lives in
  `schema.ts` (`ensureSchema()`); `mod.ts`/`guilds.ts` import the table-name
  constants from there.
- **The shared `/callback` must stay singular.** The guild comes from the state
  nonce (`consumeState` returns it), not the path.
- **Never echo secrets.** The admin API redacts `turnstileSecretKey` and
  compares `ADMIN_SECRET` in constant time (`admin.ts`).
- **Legacy upgrades are automatic.** `seedLegacyGuildFromEnv()` (config.ts) runs
  once to seed a `captcha_guilds` row from old single-guild env vars and migrate
  the log + blob IDs. Keep it idempotent.

### Verifying changes

Type-check + lint are the local ceiling (the Discord/Turnstile/SQLite paths need
a Val Town deploy): `deno check --allow-import *.ts` and `deno lint *.ts`. The
`no-import-prefix` lint warnings on `https://esm.town` / `https://esm.sh` imports
are expected (Val Town requires those URL imports) — ignore them.
